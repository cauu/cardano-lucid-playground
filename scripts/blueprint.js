...
function resolveSchema(schema, definitions) {
  if (schema.items) {
    if (schema.items instanceof Array) {
      return {
        ...schema,
        items: schema.items.map((item) => resolveSchema(item, definitions))
      };
    } else {
      return {
        ...schema,
  ...
  switch (shapeType) {
    ...
    case 'constructor': {
      if (isVoid(schema)) {
        ...
        return `{${schema.fields.map((field) => `${field.title || 'wrapper'}:${schemaToType(field)}`).join(';')}}`;
      }
    }
    case 'enum': {
          ...
          entry.fields.length === 0
            ...
            : `{${entry.title}: ${
                entry.fields[0].title
                  ? `{${entry.fields.map((field) => [field.title, schemaToType(field)].join(':')).join(',')}}}`
                  : `[${entry.fields.map((field) => schemaToType(field)).join(',')}]}`
              }`
        )
        .join(' | ');
    }
